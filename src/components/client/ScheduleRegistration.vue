...
export default defineComponent({
  ...
  methods: {
    ...
    isSubscriptionCountEmpty() {
      return this.$store.state.auth.user.remainingServicesCount === 0;
    },
    registerOnSchedule() {
      UserAPI.registerOnSchedule(this.selectedEvent.id).then(
          (result) => {
            if (result.status === 200) {
              for (let idx in result.data.registrations) {
                if (result.data.registrations[idx].userId === this.$store.state.auth.user.id) {
                  this.$store.state.auth.user.scheduleRegistrations.push(result.data.registrations[idx]);
                }
              }

              this.eventInfo.event.remove();
              this.eventInfo.view.calendar.addEvent(this.getFilledEvent(result.data));
              this.$store.state.auth.user.remainingServicesCount--;
              this.closeModal();
            }
          }
      )
    },
    unregisterOnSchedule() {
      UserAPI.unregisterOnSchedule(this.selectedEvent.id).then(
          (result) => {
            this.$store.state.auth.user.scheduleRegistrations.forEach((registration, index) => {
              if (registration.scheduleId === Number(this.selectedEvent.id)) {
                this.$store.state.auth.user.scheduleRegistrations.splice(index, 1);
              }
            })
    ...
    getFilledEvent(schedule) {
      return {
        ...
        color: this.isDateSelectable(new Date(schedule.dateFrom))
            ? ScheduleService.isUserScheduledEvent(schedule.dateFrom) ? '#00b49f' : '#1E252B' : '#A9A9A9FF'
      }
    },
